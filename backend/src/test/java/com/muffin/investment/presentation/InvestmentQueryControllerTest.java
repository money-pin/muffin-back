...
class InvestmentQueryControllerTest {
    ...
    @BeforeEach
    void setUp() {
        mockMvc = standaloneSetup(new InvestmentQueryController(investmentQueryService))
                .build();
    }
    ...
    void getAsset_returnsAssetOverview() throws Exception {
        ...
        mockMvc.perform(get("/api/investments/asset").header("X-User-Id", 1L))
    ...
    void getToday_returnsScreenStatus() throws Exception {
        ...
        mockMvc.perform(get("/api/investments/today").header("X-User-Id", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.isSuccess").value(true))
