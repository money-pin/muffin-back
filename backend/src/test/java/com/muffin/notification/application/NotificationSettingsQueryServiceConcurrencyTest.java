...
class NotificationSettingsQueryServiceConcurrencyTest {
    ...
    void concurrentGetSettings_bothSucceedAndCreateExactlyOneRow() throws Exception {
        ...
        List<Callable<NotificationSettingsResponse>> tasks = IntStream.range(0, requestCount)
                .<Callable<NotificationSettingsResponse>>mapToObj(i -> () -> {
                    ready.countDown();
                    start.await();
        ...
        for (Callable<NotificationSettingsResponse> task : tasks) {
        ...
        List<NotificationSettingsResponse> results = new ArrayList<>();
